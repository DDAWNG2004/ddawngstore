...
app.post('/admin/products/update/:id', upload.any(), async (req, res) => {
    ...
    try {
        ...
        const { productName, productCode, description, category, price, salePrice, stock, sku, status, existing_variants, variants } = req.body;
        
        // Cập nhật thông tin sản phẩm
        const updateResult = await sequelize.query(`
            UPDATE products 
            SET name = ?, description = ?, price = ?, category_id = ?, sku = ?, status = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        `, {
            replacements: [productName, description, parseFloat(price), category || null, sku || null, status || 'active', productId],
            transaction
        });
        ...
        console.log('🗑️ Old variants deleted:', deleteResult);
        
        // Thêm lại variants
        const allVariants = [];
        
        // Xử lý variants hiện tại và mới (logic tương tự như add product)
        // ... (giữ lại logic xử lý variants từ file cũ)
        ...
        res.status(500).render('error', { message: 'Lỗi khi cập nhật sản phẩm: ' + error.message });
    }
});
